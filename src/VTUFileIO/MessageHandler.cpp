...
void MessageHandler::ReportExportInfo(int pointNum, int elemNum) {
	...
	QString message = QString("VTK Export SUCCESS: %1 nodes and %2 cells written.").arg(pointNum).arg(elemNum);
...
void MessageHandler::ReportExportErr(int err) {
	...
	if (!err) {
		Ins.Print("=== VTK Export Task Completed ===");
		return;
	}

	QStringList warn;
	warn.append(QString("!!! VTK Export FAILED !!!"));
