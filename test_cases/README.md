...
`SAM_H5` 目录中包含 12 个 SAM H5 文件。这些文件均已在 SAM 中实际导出为 Legacy ASCII VTK，并完成网格与结果字段对比验证。

## 测试步骤

1. 先按 `deploy/README.md` 安装 VTKToolset 运行包。
2. 启动 SAM，打开或导入 `SAM_H5` 中任意一个 `.sam.h5` 文件。
3. 切换到模型或后处理结果环境。
...
本批测试一共对比了 44 个 VTK 文件，结果为 44 个通过、0 个失败。`verified_cases.json` 记录了纳入本目录的 12 个 H5 文件名。

原目录中另有 3 个 H5 没有对应的实际 VTK 导出记录，因此未放入本测试案例目录。
